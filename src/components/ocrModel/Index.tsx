...
import { Plus, Trash2, Image, Sparkles } from "lucide-react";
import { useParams, useSearchParams } from "react-router-dom";
import { useOCRModelStore } from "@/store/ocrModelStore";
import { useModelProviderStore } from "@/store/modelProviderStore";
import CreateOCRModelModal from "@/components/ocrModel/CreateOCRModelModal";
import { OCRModelInterface } from "@/interfaces/OCRModelInterface";

function OCRModelIndex() {
  const { org_id } = useParams();
  const [searchParams, setSearchParams] = useSearchParams();

  const provider = searchParams.get("provider") || "mistral";

  const { getOCRModelProviders } = useModelProviderStore();
  const { ocrModels, getAllProviderOCRModels, deleteOCRModel } = useOCRModelStore();
  ...
  useEffect(() => {
    const loadProviders = async () => {
      const data = await getOCRModelProviders();
      setProviders(data || []);
    };
    loadProviders();
  }, []);
  ...
  useEffect(() => {
    if (org_id && provider) {
      getAllProviderOCRModels(org_id, provider);
    }
  }, [org_id, provider]);
  ...
  const handleDelete = async (id: string) => {
    ...
    await deleteOCRModel(org_id, provider, id);
    setConfirmDelete(null);
    setDoubleConfirm(null);
  };

  const isEmpty = !ocrModels || ocrModels.length === 0;

  return (
    <div className="space-y-6 max-w-[95%] mx-auto">
      {/* HEADER */}
      <h1 className="text-xl font-semibold">OCR Models</h1>
      ...
      {isEmpty ? (
        <div className="flex flex-col items-center justify-center py-24 px-6 rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50 text-center">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-xs mb-6">
            Be the first to add an OCR model under{" "}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ocrModels.map((model: OCRModelInterface) => (
            <div
              key={model.id}
      ...
      {/* MODAL */}
      <CreateOCRModelModal open={open} onClose={() => setOpen(false)} provider={provider} />
    </div>
  );
}

export default OCRModelIndex;
