...
export function VariablesDetailSection({ orgId, projectId }: VariablesDetailSectionProps) {
  ...
  useEffect(() => {
    const fetchAndMergeData = async () => {
      setLoading(true);
      try {
        // Fetch both defaults (for descriptions/keys) and project specific values concurrently
        ...
        if (defaults && projectVars) {
          const mergedData = defaults.map((def) => {
            ...
            // Check if the saved value differs from the default value
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                {/* Badge indicating if the user changed this from the default during creation */}
                {v.isModified ? (
